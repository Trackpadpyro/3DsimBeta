...
int main() {
	...
	while (window.isOpen()) {
		...
		if (runl && counter < trailsize) {
			...
			p2.push_back({xin + stepsize * param1 * (yin - xin), yin + stepsize * (xin * (param2 - zin) - yin), zin + stepsize * (xin * yin - param3 * zin)});
		...
		if (runl && counter >= trailsize) {
			...
			p2[NumOfShapesToRender + 3 + ((int)counter % (trailsize - 1))] = { xin + stepsize * param1 * (yin - xin), yin + stepsize * (xin * (param2 - zin) - yin), zin + stepsize * (xin * yin - param3 * zin)};
